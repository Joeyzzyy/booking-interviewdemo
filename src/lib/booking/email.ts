...
/**
 * Resend 郵件發送（僅服務端）。
 * 未配置 RESEND_API_KEY 時靜默跳過並打印 warning，不阻斷流程。
 */
...
function detailRows(booking: Booking): string {
  ...
  return rows
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 12px;color:#5d6b85;white-space:nowrap">${k}</td><td style="padding:6px 12px;font-weight:600">${v ?? "-"}</td></tr>`
    )
    .join("");
}
...
function wrapHtml(title: string, body: string): string {
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e3e7f1">
    ...
    <div style="padding:24px">
      ...
      ${body}
      <p style="color:#8b95ad;font-size:12.5px;margin-top:24px">${brand.nameFull} · ${brand.taglineEn}</p>
    </div>
  </div>
</body></html>`;
}
...
export async function sendVerificationCodeEmail(
  ...
  const title = purpose === "login" ? "你的登入驗證碼" : "你的綁定驗證碼";
  const subject = purpose === "login" ? `【登入驗證碼】${code}` : `【綁定驗證碼】${code}`;
  const html = wrapHtml(
    title,
    `<p style="font-size:32px;font-weight:700;letter-spacing:6px;text-align:center;color:#2a8163;margin:24px 0">${code}</p>
     <p style="text-align:center;color:#5d6b85">驗證碼 10 分鐘內有效，請勿轉發畀其他人。</p>`
  );
...
export async function sendNewOrderNotice(booking: Booking): Promise<boolean> {
  ...
  const html = wrapHtml(
    ...
    `<p>收到新訂單，請盡快到後台確認：<a href="${process.env.NEXT_PUBLIC_SITE_URL || ""}/admin?tab=bookings">管理後台</a></p>
     <table style="border-collapse:collapse;margin:16px 0">${detailRows(booking)}</table>
