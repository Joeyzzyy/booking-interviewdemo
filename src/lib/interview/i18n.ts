...
export const UI_STRINGS: Record<LocaleKey, Record<string, string>> = {
  en: {
    ...
    ruleSeconds: "Maximum {max} seconds per answer. You may re-record up to {n} times per question.",
  ...
  id: {
    ...
    ruleSeconds: "Maksimal {max} detik per jawaban. Anda dapat merekam ulang hingga {n} kali per pertanyaan.",
  ...
  tl: {
    ...
    ruleSeconds: "Hanggang {max} segundo bawat sagot. Maaaring mag-record muli nang hanggang {n} beses bawat tanong.",
  ...
  zh: {
    ...
    ruleSeconds: "每道题录制上限 {max} 秒，最多可重录 {n} 次。",
